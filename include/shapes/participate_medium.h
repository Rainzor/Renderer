//
// Created by Runze on 28/06/2023.
//
...
bool participate_medium::hit(const ray& r, double t_min, double t_max, hit_record& rec) const {
...
//        return false;
//
//    if (rec1.t < 0) { //rec1.t -> origin -> rec2.t
//        rec.t = rec2.t;
//        rec.p = r.at(rec.t);
//    } else{//origin -> rec1.t -> boundary -> rec2.t
//        rec.t = rec1.t;
//        rec.p = r.at(rec.t);
//    }
...
//    }
    hit_record rec1;
    if (!boundary->hit(r, t_min, t_max, rec1))
        return false;
    rec.p = rec1.p;
    rec.t = rec1.t;
