...
#pragma omp critical
    ...
    std::cout << std::endl << "Time Cost:"
              ...
              << duration.count()%60 << "s" << std::endl;    std::cout << "Writing to " << img_name << std::endl;
    std::cout << "Done." << std::endl;
}

color RayTracer::ray_color(const ray &r, SampleMethod method) const {
    color L(0,0,0);
    switch (method) {
...
color RayTracer::BRDF_sample(const ray &r)const {
    hit_record rec;
...
color RayTracer::NEE_sample(const ray &r,int depth,bool is_shadow) const {
...
color RayTracer::Muliti_Importance_sample(const ray &r,int depth, double emitted_weight,bool is_shadow)const{
    //TODO:处理三角形面片
